...
type CreateChangelogComposePanelProps = {
...
export function CreateChangelogComposePanel({
  composeForm,
  previewOpen,
  onPreviewOpenChange,
  submitPending,
  ...
  leadingActionSlot,
}: CreateChangelogComposePanelProps) {
  ...
  const [publishedAtLocal, category, title, summary, breaking, body] = previewValues
  ...
  const toggleSuggestedTag = (label: string) => {
    ...
    setValue(
      "tags",
      has ? prev.filter((t) => t.toLowerCase() !== key) : mergeUniqueTags(prev, [label]),
      { shouldDirty: true }
    )
  }
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-col overflow-hidden lg:min-h-0">
      ...
      <form
        className="min-h-0 flex-1 overflow-y-auto px-2 [-webkit-overflow-scrolling:touch] sm:px-3"
        onSubmit={(event) => {
